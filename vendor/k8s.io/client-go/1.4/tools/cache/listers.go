...
// Exists checks if the given PetSet exists in the store.
func (s *StoreToTAppLister) Exists(ps *gaia.TApp) (bool, error) {
	_, exists, err := s.Store.Get(ps)
	if err != nil {
		return false, err
	}
...
func (s *StoreToTAppLister) List() (tappList []gaia.TApp, err error) {
	for _, tapp := range s.Store.List() {
		tappList = append(tappList, *(tapp.(*gaia.TApp)))
	}
	return tappList, nil
}
...
func (s *StoreToTAppLister) GetPodTApps(pod *api.Pod) (tappList []gaia.TApp, err error) {
	var selector labels.Selector
	var tapp gaia.TApp
