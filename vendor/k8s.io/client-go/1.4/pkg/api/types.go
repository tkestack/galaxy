...
type VolumeSource struct {
	...
	// GaiaCloudVolume represents a gaia cloud volume mount on host and bind mount to the pod.
	GaiaCloudVolume *GaiaCloudVolumeSource `json:"gaiaCloudVolume,omitempty"`
	// GaiaRBD represents a Rados Block Device mount on the host that shares a pod's lifetime
	GaiaRBD *RBDVolumeSource `json:"gaiaRbd,omitempty"`
...
const (
	GaiaCloudVolumeAllocateNew   string = "new"
	GaiaCloudVolumeAllocateReUse string = "re-use"
)
...
type NodeStatus struct {
	...
	Topology []NUMANode `json:"topology,omitempty"`
}

type NUMANode struct {
	// Identifies a NUMA node on a single host.
	NUMANodeID int64 `json:"numaNodeId,omitempty"`
...
const (
	...
	// Ingress request, in bits (500G = 500 * 1000 * 1000 * 1000)
	ResourceRequestsIngress ResourceName = "requests.ingress"
	// Egress request, in bits (500G = 500 * 1000 * 1000 * 1000)
...
// if the label set the label "gaia_quota: quota-yes", then resource quota will not check the pod resource quota,
// but check the deployment
const (
	GAIA_QUOTA = "gaia_quota"
	QUOTA_YES  = "quota-yes"
)
