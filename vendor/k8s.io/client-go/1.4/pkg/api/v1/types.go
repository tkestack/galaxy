...
type VolumeSource struct {
	...
	// GaiaCloudVolume represents an Gaia Cloud Volume on the host and bind mount to the pod.
	GaiaCloudVolume *GaiaCloudVolumeSource `json:"gaiaCloudVolume,omitempty" protobuf:"bytes,23,opt,name=gaiaCloudVolume"`
	// RBD represents a Rados Block Device mount on the host that shares a pod's lifetime.
	// More info: http://releases.k8s.io/release-1.4/examples/volumes/rbd/README.md
	GaiaRBD *RBDVolumeSource `json:"gaiaRbd,omitempty" protobuf:"bytes,24,opt,name=gaiaRbd"`
	// LocalDisk represents a local disk request for a pod.
	LocalDisk *LocalDiskSource `json:"localDisk,omitempty" protobuf:"bytes,25,opt,name=localDisk"`
}

// Represents local disk claim for a pod
type LocalDiskSource struct {
	// Requested local disk size
	DiskSize resource.Quantity `json:"diskSize" protobuf:"bytes,1,opt,name=diskSize"`
...
// Represents a gaia cloud volume resource. A gaia cloud volume need not to be exist
// before mounting to a container, which will be created by kubelet on-demand.
type GaiaCloudVolumeSource struct {
	// how to allocate the volume for pod new/re-use
	AllocateMode string `json:"allocateMode" protobuf:"bytes,1,req,name=allocateMode"`
	// Volume size in GB
	VolumeSize int32 `json:"volumeSize" protobuf:"varint,2,req,name=volumeSize"`
	// Filesystem type to mount.
	// Must be a filesystem type supported by the host operating system.
	// Ex. "ext4", "xfs", "ntfs". Implicitly inferred to be "ext4" if unspecified.
	FSType string `json:"fsType,omitempty" protobuf:"bytes,3,opt,name=fsType"`
...
type ContainerStatus struct {
	...
	// The portmapping result on kubelet
	Ports []ContainerPort `json:"ports,omitempty" patchStrategy:"merge" patchMergeKey:"containerPort" protobuf:"bytes,9,rep,name=ports"`
}
...
type NodeStatus struct {
	...
	// Topology represents the NUMA topology of a node to aid NUMA aware.
	Topology []NUMANode `json:"topology,omitempty" protobuf:"bytes,12,rep,name=topology"`
}

type NUMANode struct {
	// Identifies a NUMA node on a single host.
	NUMANodeID int64 `json:"numaNodeId" protobuf:"varint,1,opt,name=numaNodeId"`
	...
	Capacity ResourceList `json:"capacity" protobuf:"bytes,2,opt,name=capacity"`
	// Allocatable represents the resources of a NUMA node that are available for scheduling.
	// +optional
	Allocatable ResourceList `json:"allocatable" protobuf:"bytes,3,opt,name=allocatable"`
	// CPUSet represents the physical numbers of the CPU cores
	// associated with this node.
	// Example: 0-3 or 0,2,4,6
	// The index for CPUSet is cpu id.
	CPUSet string `json:"cpuSet" protobuf:"bytes,4,opt,name=cpuSet"`
}
