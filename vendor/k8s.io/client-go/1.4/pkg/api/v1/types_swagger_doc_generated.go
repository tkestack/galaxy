...
var map_ContainerStatus = map[string]string{
	...
	"ports":        "The portmapping result on kubelet",
}
...
var map_GaiaCloudVolumeSource = map[string]string{
	"":             "Represents a gaia cloud volume resource. A gaia cloud volume need not to be exist before mounting to a container, which will be created by kubelet on-demand.",
	"allocateMode": "how to allocate the volume for pod new/re-use",
	"volumeSize":   "Volume size in GB",
	"fsType":       "Filesystem type to mount. Must be a filesystem type supported by the host operating system. Ex. \"ext4\", \"xfs\", \"ntfs\". Implicitly inferred to be \"ext4\" if unspecified.",
}

func (GaiaCloudVolumeSource) SwaggerDoc() map[string]string {
	return map_GaiaCloudVolumeSource
}
...
var map_NUMANode = map[string]string{
	"numaNodeId":  "Identifies a NUMA node on a single host.",
	"capacity":    "Capacity represents the total resources associated to the NUMA node. cpu: 4 <number of cores> memory: <amount of memory in normal page size>",
	"allocatable": "Allocatable represents the resources of a NUMA node that are available for scheduling.",
	"cpuSet":      "CPUSet represents the physical numbers of the CPU cores associated with this node. Example: 0-3 or 0,2,4,6 The index for CPUSet is cpu id.",
}

func (NUMANode) SwaggerDoc() map[string]string {
	return map_NUMANode
}
...
var map_NodeStatus = map[string]string{
	...
	"localDisks":      "List of local disks",
	...
	"topology":        "Topology represents the NUMA topology of a node to aid NUMA aware.",
}
...
var map_VolumeSource = map[string]string{
	...
	"quobyte":         "Quobyte represents a Quobyte mount on the host that shares a pod's lifetime",
	"azureDisk":       "AzureDisk represents an Azure Data Disk mount on the host and bind mount to the pod.",
	"gaiaCloudVolume": "GaiaCloudVolume represents an Gaia Cloud Volume on the host and bind mount to the pod.",
	"gaiaRbd":         "RBD represents a Rados Block Device mount on the host that shares a pod's lifetime. More info: http://releases.k8s.io/release-1.4/examples/volumes/rbd/README.md",
