...
func validateVolumeSource(source *api.VolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if source.GaiaRBD != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, field.Forbidden(fldPath.Child("gaiaRbd"), "may not specify more than 1 volume type"))
		} else {
			numVolumes++
			allErrs = append(allErrs, validateRBDVolumeSource(source.GaiaRBD, fldPath.Child("gaiaRbd"))...)
		}
	}
	...
	if source.GaiaCloudVolume != nil {
		numVolumes++
		allErrs = append(allErrs, validateGaiaCloudVolume(source.GaiaCloudVolume, fldPath.Child("gaiaCloudVolume"))...)
	}
...
var supportedGaiaCloudVolumeAllocateMode = sets.NewString(string(api.GaiaCloudVolumeAllocateNew), string(api.GaiaCloudVolumeAllocateReUse))

// Validate the gaiaCloudVolume item
func validateGaiaCloudVolume(gaiaCloudVolume *api.GaiaCloudVolumeSource, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if gaiaCloudVolume.AllocateMode == "" {
		allErrs = append(allErrs, field.Required(fldPath.Child("allocateMode"), ""))
	} else if !supportedGaiaCloudVolumeAllocateMode.Has(gaiaCloudVolume.AllocateMode) {
		allErrs = append(allErrs, field.NotSupported(fldPath.Child("allocateMode"), gaiaCloudVolume.AllocateMode, supportedGaiaCloudVolumeAllocateMode.List()))
	}
	if gaiaCloudVolume.VolumeSize == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("volumeSize"), ""))
	}
	return allErrs
}
