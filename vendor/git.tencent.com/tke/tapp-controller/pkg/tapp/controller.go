...
import (
	...
	clientset "git.tencent.com/tke/tapp-controller/pkg/client/clientset/versioned"
	tappscheme "git.tencent.com/tke/tapp-controller/pkg/client/clientset/versioned/scheme"
	informers "git.tencent.com/tke/tapp-controller/pkg/client/informers/externalversions"
	listers "git.tencent.com/tke/tapp-controller/pkg/client/listers/tappcontroller/v1"
	"git.tencent.com/tke/tapp-controller/pkg/hash"
	"git.tencent.com/tke/tapp-controller/pkg/util"
	...
	tappInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, cur interface{}) {
			...
			if oldTApp.Status.Replicas != curTApp.Status.Replicas {
				glog.V(4).Infof("Observed updated replica count for tapp %s: %d->%d", util.GetTAppFullName(curTApp), oldTApp.Status.Replicas, curTApp.Status.Replicas)
			}
...
func (c *Controller) getTAppForPod(pod *corev1.Pod) (*tappv1.TApp, error) {
	...
	if len(tapps) == 0 {
		return nil, fmt.Errorf("could not find tapps for pod %s in namespace %s with labels: %v", pod.Name, pod.Namespace, pod.Labels)
	}
...
func (c *Controller) Sync(key string) error {
	...
	if isTAppFinished(tapp) && tapp.Generation == tapp.Status.ObservedGeneration &&
		...
		glog.Errorf("Tapp %s has finished, replica: %d, status: %s", util.GetTAppFullName(tapp), tapp.Spec.Replicas, tapp.Status.AppStatus)
		return nil
	}
...
func shouldPodMigrate(tapp *tappv1.TApp, pod *corev1.Pod, id string) (bool, error) {
	...
	if err != nil {
		return true, fmt.Errorf("failed to get pod template from tapp %s with id %s: %v", util.GetTAppFullName(tapp), id, err)
	}
	...
	if len(pod.Spec.NodeName) != 0 && len(pod.Status.InitContainerStatuses) == 0 && len(pod.Status.ContainerStatuses) == 0 {
		return true, nil
	}
...
func (c *Controller) instanceToSync(tapp *tappv1.TApp, pods []*corev1.Pod) (add, del, forceDel, update []*Instance) {
	...
	for _, id := range running.List() {
		if pod, ok := podMap[id]; !ok {
			// pod not exist, should create it
			// pod store maybe out of date, instanceSyncer should not
			// delete existing instance when process AddInstance
			...
			// Pod is pending/running, check whether template hash changes.
...
func rollUpdateFilter(tapp *tappv1.TApp, pods []*corev1.Pod, updates, dels []*Instance, updating map[string]struct{}) []*Instance {
	...
	for i := 0; i < int(tapp.Spec.Replicas); i++ {
		...
		if isPodInActive(pod) {
			continue
		}

		if updating != nil {
			if _, ok := updating[id]; ok {
				continue
			}
		}
...
func getUpdatingPods(pods []*corev1.Pod) map[string]struct{} {
