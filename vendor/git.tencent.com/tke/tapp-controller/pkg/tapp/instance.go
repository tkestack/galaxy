...
func (p *ApiServerInstanceClient) recordClaimEvent(verb string, ins *Instance, claim *corev1.PersistentVolumeClaim, err error) {
	if err == nil {
...
func mergePod(real, excepted *corev1.Pod) {
	newContainers := make([]corev1.Container, len(excepted.Spec.Containers))
...
func (p *ApiServerInstanceClient) event(obj runtime.Object, reason, msg string, err error) {
	if err != nil {
		p.Recorder.Eventf(obj, corev1.EventTypeWarning, fmt.Sprintf("Failed%v", reason), fmt.Sprintf("%v, error: %v", msg, err))
	} else {
