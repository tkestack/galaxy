...
type UnstructuredBuilder interface {
	...
	WithLabels(labels map[string]string) UnstructuredBuilder
...
func (u *unstructuredBuilder) WithLabels(labels map[string]string) UnstructuredBuilder {
	r := *u
	r.labels = labels
	return &r
}
